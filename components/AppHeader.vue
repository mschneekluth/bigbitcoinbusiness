<template>
  <header
    ...
    :class="scrolled ? 'bg-base-100/95 backdrop-blur-lg border-b border-base-300/60 shadow-lg shadow-black/10' : 'bg-base-100/80 backdrop-blur-md border-b border-base-300/30'"
  >
      ...
      <div class="flex items-center justify-between h-[68px] lg:h-[76px]">
        ...
        <NuxtLink to="/" class="flex items-center gap-3 shrink-0 group">
          ...
          <div class="hidden sm:block leading-tight">
            <div class="text-[11px] lg:text-xs font-bold tracking-[0.3em] uppercase text-base-content/90">
              Big Bitcoin
            </div>
            <div class="text-[11px] lg:text-xs font-bold tracking-[0.3em] uppercase text-primary -mt-0.5">
              Business
            </div>
          </div>
        </NuxtLink>
        ...
        <nav class="hidden lg:flex items-center gap-0.5">
          ...
          >
            {{ item.label }}
            <span class="absolute bottom-1 left-1/2 -translate-x-1/2 w-0 h-[2px] bg-primary rounded-full group-hover:w-4 transition-all duration-300 opacity-0 group-hover:opacity-100" />
          </NuxtLink>
        </nav>
    ...
    <Transition name="slide-down">
      <div v-if="mobileMenuOpen" class="lg:hidden border-t border-base-300/50 bg-base-100">
